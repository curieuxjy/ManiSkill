...
python study_allegro/scripts/01_explore_env.py
```
...
```bash
# 비디오 저장 (디스플레이 없어도 됨)
python study_allegro/scripts/02_visualize.py --mode video

# GUI 뷰어 (디스플레이 필요)
python study_allegro/scripts/02_visualize.py --mode viewer
```
...
```bash
# TensorBoard (로컬) — 로그가 study_allegro/runs/ 하위에 저장됨
tensorboard --logdir=study_allegro/runs/
...
학습 결과(체크포인트, TensorBoard 로그, wandb 로그)는 모두 `study_allegro/runs/`와 `study_allegro/wandb/` 하위에 저장됩니다.
...
```bash
bash study_allegro/scripts/04_train_sac.sh 0 64 2000000
```
...
```bash
# PPO 모델 평가 (체크포인트 경로는 study_allegro/ 기준 상대경로)
...
```bash
bash study_allegro/scripts/03_train_ppo.sh 0 256 5000000
bash study_allegro/scripts/03_train_ppo.sh 1 256 10000000
bash study_allegro/scripts/03_train_ppo.sh 2 256 10000000
bash study_allegro/scripts/03_train_ppo.sh 3 256 15000000
```
...
### 로봇 에이전트
- `mani_skill/agents/robots/allegro_hand/allegro.py` — `AllegroHandRight`, `AllegroHandLeft` (16 DOF, 4손가락 × 4관절)
- `mani_skill/agents/robots/allegro_hand/allegro_touch.py` — `AllegroHandRightTouch` (FSR 촉각 센서 16개 추가)

### 태스크 환경
- `mani_skill/envs/tasks/dexterity/rotate_single_object_in_hand.py` — 손 위에서 물체 회전 (Level 0~3)

### URDF/에셋
- `mani_skill/assets/robots/allegro/` — URDF 파일 및 메시

### RL 학습 베이스라인
- `examples/baselines/ppo/ppo.py` — PPO (state-based)
- `examples/baselines/sac/sac.py` — SAC (state-based)

## 환경 구조 이해

Allegro Hand 환경의 핵심 흐름:

```
gym.make("RotateSingleObjectInHandLevel0-v1")
  → RotateSingleObjectInHandLevel0 (BaseEnv 상속)
    → robot_uids="allegro_hand_right_touch" (촉각 센서 포함)
    → _load_scene(): 테이블 + 큐브/YCB 물체 생성
    → _initialize_episode(): 물체/로봇 초기 위치 설정, 회전 목표축 결정
    → evaluate(): 누적 회전각, 낙하 여부, 성공 판단
    → compute_dense_reward(): 회전보상 + 속도/낙하/에너지 페널티 + 손가락-물체 거리 보상
```
