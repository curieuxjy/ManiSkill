...
```
study_allegro/envs/allegro_rotate.py
...
python study_allegro/scripts/run_with_patch.py \
...
python study_allegro/scripts/08_gpu_visual_data.py --section explore     # obs_mode별 데이터 구조
...
```bash
python study_allegro/scripts/10_gpu_heterogeneous.py                         # 전체 실행
...
BaseEnv (mani_skill/envs/sapien_env.py)
  └── RotateSingleObjectInHand           ← 원본 base (partial reset 버그 있음)
        ...
        ├── ...
        └── AllegroRotate                ← study_allegro 커스텀 (_initialize_actors 오버라이드)
