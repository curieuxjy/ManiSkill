...
def explore_backends():
    ...
    주의: PhysX는 한 프로세스에서 CPU → GPU 전환이 불가능하다.
    CPU PhysX가 먼저 초기화되면 이후 GPU PhysX를 활성화할 수 없으므로,
    GPU 환경을 먼저 생성한 뒤 CPU 환경 정보는 설명으로 대체한다.
    """
    print_header("Section 1: CPU vs GPU 시뮬레이션 백엔드")

    # ── CPU 백엔드 설명 (직접 실행하면 이후 GPU 사용 불가) ──
    print("num_envs=1, sim_backend='auto':")
    print("  (CPU/GPU PhysX는 한 프로세스에서 공존 불가 → 설명으로 대체)")
    print("  gpu_sim_enabled = False")
    print("  device          = cpu")
    print("  num sub-scenes  = 1")
    print("  PhysX backend   = PhysxCpuSystem")
    print("  obs device      = cpu")
    print()
    ...
    for num_envs, sim_backend in gpu_configs:
        env = gym.make(
            "PickCube-v1",
            num_envs=num_envs,
            obs_mode="state",
    ...
    print("  - ⚠ CPU와 GPU PhysX는 한 프로세스에서 공존 불가 (GPU 먼저 활성화 필요)")
